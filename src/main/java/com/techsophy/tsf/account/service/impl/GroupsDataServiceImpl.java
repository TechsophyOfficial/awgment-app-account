...
import com.techsophy.tsf.account.exception.RealmAcessException;
...
/**
 * Service implementation for managing groups data.
 * This class provides methods to retrieve, create, update, and delete group definitions
 * while interacting with Keycloak for role assignments.
 */
@RefreshScope
@Service
...
public class GroupsDataServiceImpl implements GroupsDataService {
    ...
    /**
     * Retrieves all groups with optional search query and sorting.
     *
     * @param q Query string for filtering groups.
     * @param sort Sorting parameters.
     * @param deploymentIdList Comma-separated deployment IDs for filtering.
     * @return List of group data schemas.
     */
    @Override
     ...
     * @return Paginated response containing group data.
     */
    @Override
    ...
    /**
     * Retrieves a group by its ID and fetches associated roles from Keycloak.
     *
     * @param id Group ID.
     * @return Group data schema with assigned roles.
     */
    @Override
    public GroupsDataSchema getGroupById(String id) throws JsonProcessingException {
        ...
        GroupDefinition groupsDefinition = groupRepository.findById(new BigInteger(id)).orElseThrow(() -> new InvalidInputException(UNABLE_TO_GET_GROUPS_WITH_ID, globalMessageSource.get(UNABLE_TO_GET_GROUPS_WITH_ID, id)));
    ...
    public GroupsDataSchema saveGroup(GroupsData groupsData) {
        try {
            ...
            if (StringUtils.isEmpty(groupsData.getId())) {
                ...
                return this.objectMapper.convertValue(groupDefinition, GroupsDataSchema.class).withCreatedOn(groupDefinition.getCreatedOn());
            }
            ...
            List<GroupDefinition> update = groupsDefinition.stream().filter(data -> !String.valueOf(data.getId()).equals(groupsData.getId()) && data.getName().equals(groupsData.getName())).collect(Collectors.toList());
            if (!update.isEmpty()) {
            ...
            if (groupRepository.existsById(new BigInteger(groupsData.getId()))) {
                ...
                groupRepository.save(groupDefinition);
                return this.objectMapper.convertValue(groupDefinition, GroupsDataSchema.class).withUpdatedOn(groupDefinition.getUpdatedOn());
            } else {
    ...
    /**
     * Paginates group definitions and maps them to a response payload.
     *
     * @param groupsDefinitionPageable Page of group definitions.
    ...
    public PaginationResponsePayload groupsPagination(Page<GroupDefinition> groupsDefinitionPageable) {
        Stream<Map<String, Object>> groupsDataSchema = groupsDefinitionPageable.stream().map(groupDefinitionData -> {
